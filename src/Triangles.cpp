...
float colorparam[4][3] = { {193, 186, 186}, {214, 214, 214}, {227, 230, 232}, {202, 205, 206} };
...
void ofApp::setup(){
	...
	for(int cnt = 0; cnt < PIECES; cnt++){
		for(int ct = 0; ct < 3; ct++){
			...
			TrianglePositionVector[cnt][ct].y = ofRandom(YBOADER, ofGetHeight() - YBOADER* 2);
...
void ofApp::draw(){
	for (int i = 0; i < PIECES; i++){
		...
		triangle.drawTriangle(TrianglePositionVector[i][0].x, TrianglePositionVector[i][0].y, 
			...
			TrianglePositionVector[i][2].y, OUTLINE, colorparam[i % 4], pattern[i], alpha[i]);
		ofPopMatrix();
	}
}
